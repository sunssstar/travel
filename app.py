...
def generate_gpt4_response(query: str, context: Optional[str] = None, metadata: Optional[Dict[str, str]] = None) -> str:
    ...
    try:
        ...
        f"{top_document.metadata.get('답변', 'N/A')}을 기반으로 문장을 만드세요."
        )   
...
# 3-2. 파인튜닝 모델 
# 토큰값 조정하기
def generate_custom_model_response(query: str, context: Optional[str] = None, metadata: Optional[Dict[str, str]] = None, max_tokens: int = 40) -> str:
    try:
        documents = simple_search(query)
        if documents:
            # 첫 번째 문서의 메타데이터를 가져옴
        ...
        if max_tokens > 120:  # 토큰 수가 많을 경우
            prompt = (
                f"Question: {query}\n\n"
                "Provide a professional and medically accurate response in Korean to the question above. "
                f"Focus on the most relevant aspects of {top_document.metadata.get('질병', 'N/A')}. "
                f"Based on this sentence: '{top_document.metadata.get('답변', 'N/A')}', construct your response. "
                "Ensure the response is clear, well-structured, and demonstrates your expertise in the subject matter. "
                "Avoid repetition and ensure each sentence adds value to the user's understanding. "
                "Separate each paragraph with two new lines for readability. "
                "Do not include any part of this prompt in the output. "
                "Do not include unnecessary details, sources, translation information, or irrelevant content. "
                "The answer should be provided entirely in Korean without any English.\n\n"
                "답변임!!:"
            )
            ...
            prompt = (
                f"Question: {query}\n\n"
                "Provide a professional and medically accurate response to the question above. "
                f"Focus on the most relevant aspects of {top_document.metadata.get('질병', 'N/A')}. "
                f"Based on this sentence: '{top_document.metadata.get('답변', 'N/A')}', construct your response. "
                "If the response exceeds the token limit and sentences are cut off, do not output the incomplete sentence. "
                "Answer concisely. "  
                "Ensure the response is completed within the 40-token limit. "  
                "Do not include the prompt instructions in the output. "  
                "The answer should be only in Korean. "  
                "Do not include unnecessary details, sources, translation information, or irrelevant content.\n\n"
                "답변임!!:"
            )
...
def generate_response(query: str, context: Optional[str] = None, metadata: Optional[Dict[str, str]] = None, max_tokens: int =40) -> Tuple[str, str, float]:
    try:
        ...
        if st.session_state.model_selected == "GPT-4" and st.session_state.gpt_usage_count < MAX_GPT_USAGE:
            ...
            # GPT-4 모델 사용 시 max_tokens를 설정하지 않음
            ...
            if max_tokens == 100:  # 짧은 답변의 경우에만 추가
                    response += " ....(자세히)" 
...
def process_query(query: str, chat_history: List[Dict[str, str]]) -> Tuple[str, str, float]:
    ...
    try:
        ...
        if model == "Custom Model":
            logging.info(f"사용된 토큰 수: {100 if st.session_state.model_selected == 'Custom-200' else 800}")
...
def main():
    ...
    if user_input:
        ...
        with st.spinner("답변 생성 중..."):
            response, model, processing_time = generate_response(query=user_input, max_tokens=100)
